...
--
-- price_at_purchase captures the retail price at order time. Later
-- product price changes must not rewrite historical order totals.
...
-- Uses the product's current retail price as the historical snapshot,
-- which is the best we can do here. Anything booked after V3 gets the
-- real snapshot. (No-op when orders is empty, which is the typical
-- dev/test case.)
